...
updateCore() {
    ...
    git remote add c9 https://github.com/c9open/core 2> /dev/null || true
    git fetch c9
...
installGlobalDeps() {
    if ! [[ -f ~/.c9/installed ]]; then
        if [[ $os == "windows" ]]; then
            URL=https://raw.githubusercontent.com/cloud9ideopen/sdk-deps-win32
        else
            URL=https://raw.githubusercontent.com/c9open/install
        fi    
